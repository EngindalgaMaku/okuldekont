...
async function main() {
  console.log('🚀 Admin fonksiyonları düzeltiliyor...');
  ...
  try {
    ...
    const sqlFilePath = path.join(__dirname, 'fix-admin-functions.sql');
    const sql = fs.readFileSync(sqlFilePath, 'utf8');
    console.log(`✅ SQL betiği okunuyor: ${path.basename(sqlFilePath)}`);

    await client.query(sql);
    console.log('✅ Admin fonksiyonları başarıyla düzeltildi.');

  } catch (err) {
    console.error('❌ Admin fonksiyonları düzeltilirken hata oluştu:', err);
