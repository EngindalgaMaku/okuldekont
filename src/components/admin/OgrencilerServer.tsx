...
import StudentHistoryModalFixed from "@/components/admin/StudentHistoryModalFixed";
import OgrencilerPrintClient from "@/components/admin/OgrencilerPrintClient";
import OgrencilerActions from "@/components/admin/OgrencilerActions";
...
export default function OgrencilerServer({
  searchParams,
}: OgrencilerServerProps) {
  ...
  const [editOgrenciFormData, setEditOgrenciFormData] =
  ...
  const handleOgrenciEkle = async () => {
    if (
      !ogrenciFormData.ad.trim() ||
      !ogrenciFormData.soyad.trim() ||
      !ogrenciFormData.no.trim() ||
      !ogrenciFormData.sinif ||
      !ogrenciFormData.alanId
    ) {
  ...
  const handleTopluFormDegisiklik = (
    index: number,
    field: string,
    value: string
  ) => {
  ...
  const handleTopluOgrenciEkle = async () => {
    ...
    for (let i = 0; i < topluOgrenciler.length; i++) {
      ...
      if (!ogrenci.soyad.trim())
        hatalar.push(`${i + 1}. satır: Soyad gereklidir`);
      if (!ogrenci.no.trim())
        hatalar.push(`${i + 1}. satır: Okul numarası gereklidir`);
      if (!ogrenci.sinif.trim())
        hatalar.push(`${i + 1}. satır: Sınıf gereklidir`);
      if (!ogrenci.alanId.trim())
        hatalar.push(`${i + 1}. satır: Alan gereklidir`);
    }
  ...
  const handleCsvUpload = async () => {
    ...
    try {
      const students = csvPreview.map((row) => ({
        ...
        parentName:
          row["Veli Adı"] || row["veli_adi"] || row["PARENT_NAME"] || null,
        parentPhone:
          row["Veli Telefon"] ||
          row["veli_telefon"] ||
          row["PARENT_PHONE"] ||
      ...
      students.forEach((student, index) => {
        if (!student.name.trim())
          hatalar.push(`${index + 2}. satır: Ad gereklidir`);
        if (!student.surname.trim())
          hatalar.push(`${index + 2}. satır: Soyad gereklidir`);
        if (!student.number.trim())
          hatalar.push(`${index + 2}. satır: Numara gereklidir`);
        if (!student.className.trim())
          hatalar.push(`${index + 2}. satır: Sınıf gereklidir`);
        if (!student.alanId.trim())
          hatalar.push(`${index + 2}. satır: Alan ID gereklidir`);
      });
  ...
  const handleOgrenciGuncelle = async () => {
    if (
      !selectedOgrenci ||
      !editOgrenciFormData.ad.trim() ||
      !editOgrenciFormData.soyad.trim() ||
    ...
    try {
      ...
      const currentResponse = await fetch(
        `/api/admin/students/${selectedOgrenci.id}`
      );
      let currentData: any = {};
      ...
      const fieldMappings = [
        ...
        {
          current: currentData.surname,
          new: updateData.surname,
          field: "surname",
          type: "PERSONAL_INFO_UPDATE",
        },
        {
          current: currentData.number,
          new: updateData.number,
          field: "number",
          type: "SCHOOL_INFO_UPDATE",
        },
        {
          current: currentData.className,
          new: updateData.className,
          field: "className",
          type: "SCHOOL_INFO_UPDATE",
        },
        {
          current: currentData.tcNo,
          new: updateData.tcNo,
          field: "tcNo",
          type: "PERSONAL_INFO_UPDATE",
        },
        ...
        {
          current: currentData.parentName,
          new: updateData.parentName,
          field: "parentName",
          type: "PARENT_INFO_UPDATE",
        },
        {
          current: currentData.parentPhone,
          new: updateData.parentPhone,
          field: "parentPhone",
          type: "PARENT_INFO_UPDATE",
        },
        {
          current: currentData.email,
          new: updateData.email,
          field: "email",
          type: "CONTACT_INFO_UPDATE",
        },
      ];
  ...
  const handleModalOgrenciSilOnayla = async () => {
    ...
    try {
      const response = await fetch(
        `/api/admin/students?id=${selectedOgrenci.id}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleFesihOnayla = async () => {
    ...
    try {
      const response = await fetch(
        `/api/admin/students/${selectedOgrenci.id}/assign-company`,
        {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Tüm alanlardan öğrencileri görüntüleyin ve yönetin (
            {totalOgrenciler})
          </p>
        </div>
        <div className="flex items-center gap-3 w-full sm:w-auto">
          {/* Single Actions Dropdown containing all operations */}
          <OgrencilerActions
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {ogrenci.company ? (
                          <div className="text-sm">
                            <div className="text-gray-900 font-medium">
                              {ogrenci.company.name}
                            </div>
                            <div className="text-gray-500">
                              {ogrenci.company.contact}
                            </div>
                          </div>
                        ) : (
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {ogrenci.company?.teacher ? (
                          <div className="text-sm">
                            <div className="text-gray-900 font-medium">
                              {ogrenci.company.teacher.name}{" "}
                              {ogrenci.company.teacher.surname}
                            </div>
                            <div className="text-gray-500 text-xs">
                              {ogrenci.company.teacher.alan?.name ||
                                "Alan Belirtilmemiş"}
                            </div>
                          </div>
                        ) : (
                  ...
                  <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                    <div>
                      <p className="text-sm text-gray-700">
                        ...
                        {" - "}
                        <span className="font-medium">
                          {Math.min(currentPage * perPage, totalOgrenciler)}
                        </span>
                        {" / "}
            ...
            <div className="flex items-center justify-between pb-4 border-b border-gray-200">
              <div>
                <h3 className="text-lg font-semibold text-gray-900">
                  {selectedOgrenci.ad} {selectedOgrenci.soyad} - Bilgileri
                  Düzenle
                </h3>
                ...
                <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                  <div className="text-red-800 text-sm">
                    ...
                    <div className="mb-2">
                      ...
                      adlı öğrenciyi kalıcı olarak silmek üzeresiniz.
                    </div>
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-3">
                      <div className="text-yellow-800 text-xs">
                        <div className="font-semibold mb-1">
                          Bu işlem geri alınamaz ve aşağıdaki veriler
                          kaybolacak:
                        </div>
                    ...
                    <div className="mb-3">
                      <div className="text-sm font-medium text-gray-700 mb-2">
                        Silme işlemini onaylamak için aşağıdaki kutuya "
                        <span className="font-bold text-red-600">onay</span>"
                        yazın:
                      </div>
                      <input
                        type="text"
      ...
      <Modal
        isOpen={topluOgrenciModalOpen}
        onClose={() => setTopluOgrenciModalOpen(false)}
        ...
        <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      value={ogrenci.ad}
                      onChange={(e) =>
                        handleTopluFormDegisiklik(index, "ad", e.target.value)
                      }
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={ogrenci.no}
                      onChange={(e) =>
                        handleTopluFormDegisiklik(index, "no", e.target.value)
                      }
                      className="w-full px-2 py-1.5 text-sm border border-gray-300 rounded-md focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500"
                      placeholder="Örn: 1234"
                    />
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t">
            ...
            >
              {submitLoading
                ? "Ekleniyor..."
                : `${topluOgrenciler.length} Öğrenciyi Ekle`}
            </button>
          </div>
        </div>
      </Modal>
        ...
        <div className="space-y-4">
            ...
            <div className="bg-red-50 border border-red-200 rounded-lg p-3">
              <div className="text-red-800 text-sm">
                <div className="font-medium">
                  🎓 Öğrenci: {selectedOgrenci.ad} {selectedOgrenci.soyad}
                </div>
                <div className="mt-1">
                  🏢 İşletme: {selectedOgrenci.company?.name}
                </div>
                {selectedOgrenci.company?.teacher && (
                  <div className="mt-1">
                    👨‍🏫 Koordinatör: {selectedOgrenci.company.teacher.name}{" "}
                    {selectedOgrenci.company.teacher.surname}
                  </div>
                )}
              </div>
            </div>
          ...
          <div>
            ...
            >
              ...
              <option value="İşletme koşullarının uygun olmaması">
                İşletme koşullarının uygun olmaması
              </option>
        ...
        <div className="space-y-4">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
            <div className="text-blue-800 text-sm">
              ...
              <div className="text-xs space-y-1">
                ...
                <div>
                  • Opsiyonel: TC, Telefon, Veli Adı, Veli Telefon, Email
                </div>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t">
            ...
            >
              {csvLoading
                ? "Yükleniyor..."
                : `${csvPreview.length} Öğrenciyi Ekle`}
            </button>
          </div>
        </div>
      </Modal>
