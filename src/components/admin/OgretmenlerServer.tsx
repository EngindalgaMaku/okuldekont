...
export default async function OgretmenlerServer({ searchParams }: Props) {
  const { ogretmenler, alanlar, pagination } = await getOgretmenlerData(
    searchParams
  );
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-4 sm:space-y-0">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
          {/* Search Form */}
          <form
            action="/admin/ogretmenler"
            method="GET"
            className="flex flex-col sm:flex-row sm:items-center gap-2"
          >
            {/* Hidden inputs to preserve other params */}
            {searchParams.alan && (
              <input type="hidden" name="alan" value={searchParams.alan} />
            )}
      ...
      {ogretmenler.length === 0 ? (
        ...
        <OgretmenlerTableClient ogretmenler={ogretmenler} alanlar={alanlar} />
      )}
