...
const fieldOptions = [
  ...
  { id: 'number', label: 'Okul Numarası' },
  { id: 'className', label: 'Sınıf' },
  { id: 'tcNo', label: 'TC Kimlik No' },
  { id: 'phone', label: 'Telefon' },
  { id: 'email', label: 'E-posta' },
  { id: 'parentName', label: 'Veli Adı' },
  { id: 'parentPhone', label: 'Veli Telefon' },
  { id: 'alan', label: 'Alan' },
  { id: 'company', label: 'İşletme' },
  { id: 'teacher', label: 'Koordinatör Öğretmen' },
  { id: 'status', label: 'Durum' },
]

export default function StudentExportOptionsModal({ isOpen, onClose }: Props) {
  const [isLoading, setIsLoading] = useState(false)
  const [format, setFormat] = useState('default')
  const [selectedFields, setSelectedFields] = useState<string[]>(['name', 'surname', 'number', 'className', 'alan'])
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const filter_alan = (document.getElementById('filter_alan') as HTMLSelectElement).value
      const filter_sinif = (document.getElementById('filter_sinif') as HTMLSelectElement).value
      const filter_status = (document.getElementById('filter_status') as HTMLSelectElement).value
      ...
      const response = await fetch('/api/admin/students/export', {
        ...
        body: JSON.stringify({
          ...
          sort,
          filters: {
            alan: filter_alan !== 'all' ? filter_alan : null,
            sinif: filter_sinif !== 'all' ? filter_sinif : null,
      ...
      toast.success('Öğrenciler başarıyla dışa aktarıldı.')
      onClose()
    } catch (error) {
      console.error(error)
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center">
      <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-lg">
        <h2 className="text-xl font-bold mb-4">Öğrencileri Dışa Aktar</h2>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            >
              ...
              <option value="number_asc">Numaraya Göre (1-9)</option>
              <option value="number_desc">Numaraya Göre (9-1)</option>
              <option value="class_asc">Sınıfa Göre</option>
              <option value="alan_asc">Alana Göre</option>
            </select>
          </div>
          ...
          <div>
            ...
            <div className="mt-2 space-y-3">
              <div>
                <label htmlFor="filter_alan" className="block text-xs font-medium text-gray-600">Alan</label>
                <select
                  id="filter_alan"
                  name="filter_alan"
                ...
                >
                  <option value="all">Tüm Alanlar</option>
                  {/* Bu seçenekler API'den gelebilir */}
                </select>
              </div>
              
              <div>
                <label htmlFor="filter_sinif" className="block text-xs font-medium text-gray-600">Sınıf</label>
                <select
                  id="filter_sinif"
                  name="filter_sinif"
                  className="mt-1 block w-full pl-3 pr-10 py-2 text-sm border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 rounded-md"
              ...
              <div>
                <label htmlFor="filter_status" className="block text-xs font-medium text-gray-600">Durum</label>
                <select
                  id="filter_status"
                  name="filter_status"
                  className="mt-1 block w-full pl-3 pr-10 py-2 text-sm border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 rounded-md"
