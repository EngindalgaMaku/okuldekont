...
export default function PrintOptionsModal({
  ...
  const [totalTeacherCount, setTotalTeacherCount] = useState(
    ogretmenler.length
  );

  const handlePrintOptionChange = (
    option: keyof PrintOptions,
  ...
  const fetchAllTeachers = async (): Promise<void> => {
    ...
    try {
      ...
      if (searchParams.alan && searchParams.alan !== "all")
        apiParams.set("alan", searchParams.alan);
        ...
        if (data.success && data.ogretmenler) {
          const adaptedTeachers = data.ogretmenler.map((teacher: any) => ({
            ...
            alanlar: teacher.alan
              ? { id: teacher.alan.id, ad: teacher.alan.name }
              : null,
            koordinatorluklar:
              teacher.companies?.map((comp: any) => ({
                isletme: { id: comp.id, name: comp.name },
              })) || [],
            stajlar:
              teacher.stajlar?.map((staj: any) => ({
                student: staj.student,
                isletme: staj.company,
              })) || [],
  ...
  const generatePrintContent = () => {
    ...
    const filteredTeachers = allTeachers.sort((a, b) => {
      const alanA = a.alanlar?.ad || "ZZZ";
      const alanB = b.alanlar?.ad || "ZZZ";
    ...
    if (searchParams.alan && searchParams.alan !== "all") {
      const fieldName = filteredTeachers.find(
        (t) => t.alanlar?.id === parseInt(searchParams.alan!)
      );
      if (fieldName) {
        const fieldDisplayName = fieldName.alanlar?.ad;
  ...
  const handlePrint = async () => {
    ...
    if (allTeachers.length <= ogretmenler.length) {
      try {
        ...
        toast.error(
          "Veriler yüklenirken hata oluştu. Mevcut sayfa verileri ile yazdırılacak."
        );
      }
    }
