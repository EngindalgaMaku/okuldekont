...
export default function OgretmenDekontListesi({
  dekontlar,
  onUpdate,
}: OgretmenDekontListesiProps) {
  const [selectedDekont, setSelectedDekont] = useState<DekontDetay | null>(
    null
  );
  ...
  const updateDekontStatus = async (
    dekontId: string,
    ...
    try {
      ...
      if (response.ok) {
        toast.success(
          status === "APPROVED" ? "Dekont onaylandı" : "Dekont reddedildi"
        );
        onUpdate?.();
      } else {
  ...
  const downloadFile = async (dekont: DekontDetay) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `${dekont.ogrenci_ad_soyad}_${aylar[dekont.ay - 1]}_${
          dekont.yil
        }.pdf`;
  ...
  return (
    <div className="overflow-x-auto">
                ...
                <div className="space-y-1">
                  <div className="flex items-center">
                    ...
                    <span className="font-semibold text-gray-900">
                      {dekont.ogrenci_ad_soyad}
                    </span>
                  </div>
                  {dekont.ogrenci_sinif && (
                    <div className="text-sm text-gray-600">
                      Sınıf: {dekont.ogrenci_sinif}
                    </div>
                  )}
                  {dekont.ogrenci_no && (
                    <div className="text-sm text-gray-600">
                      No: {dekont.ogrenci_no}
                    </div>
                  )}
                  {dekont.ogrenci_alan && (
              ...
              <td className="px-6 py-4">
                <div className="space-y-1">
                  ...
                  {dekont.isletme_yetkili && (
                    <div className="text-sm text-gray-600">
                      Yetkili: {dekont.isletme_yetkili}
                    </div>
                  )}
                  {dekont.isletme_telefon && (
                    <div className="flex items-center text-sm text-blue-700">
                      ...
                      <a
                        href={`tel:${dekont.isletme_telefon}`}
                  ...
                  {dekont.isletme_email && (
                    <div className="flex items-center text-sm text-blue-700">
                      ...
                      <a
                        href={`mailto:${dekont.isletme_email}`}
                        className="hover:underline"
                      >
                        {dekont.isletme_email}
                      </a>
              ...
              <div>
                ...
                <div className="space-y-1 text-sm">
                  <p>
                    ...
                    {selectedDekont.ogrenci_ad_soyad}
                  </p>
                  {selectedDekont.ogrenci_sinif && (
                    <p>
                      <span className="font-medium">Sınıf:</span>{" "}
                      {selectedDekont.ogrenci_sinif}
                    </p>
                  )}
                  {selectedDekont.ogrenci_no && (
                    <p>
                      <span className="font-medium">No:</span>{" "}
                      {selectedDekont.ogrenci_no}
                    </p>
                  )}
                  {selectedDekont.ogrenci_alan && (
                    <p>
              ...
              <div>
                ...
                <div className="space-y-1 text-sm">
                  <p>
                    ...
                    {selectedDekont.isletme_ad}
                  </p>
                  {selectedDekont.isletme_yetkili && (
                    <p>
                      <span className="font-medium">Yetkili:</span>{" "}
                      {selectedDekont.isletme_yetkili}
                    </p>
                  )}
                  {selectedDekont.isletme_telefon && (
                    <p>
                      <span className="font-medium">Telefon:</span>{" "}
                      {selectedDekont.isletme_telefon}
                    </p>
                  )}
                  {selectedDekont.isletme_email && (
                    <p>
            ...
            <div className="border-t pt-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div>
                  ...
                  {aylar[selectedDekont.ay - 1]} {selectedDekont.yil}
                </div>
                {selectedDekont.miktar && (
                  <div>
                    <span className="font-medium">Tutar:</span>{" "}
                    {selectedDekont.miktar.toLocaleString("tr-TR")} ₺
                  </div>
                )}
                <div className="flex items-center">
      ...
      <ConfirmModal
        ...
        description={
          selectedDekont
            ? `${selectedDekont.ogrenci_ad_soyad} öğrencisinin ${
                aylar[selectedDekont.ay - 1]
              } ${selectedDekont.yil} ayına ait dekontunu onaylıyor musunuz?`
            : ""
        }
        confirmText="Onayla"
        isLoading={loading}
      ...
      <Modal
        ...
        title="Dekont Reddetme"
      >
        <div className="space-y-4">
          {selectedDekont && (
            <p className="text-sm text-gray-600">
              <strong>{selectedDekont.ogrenci_ad_soyad}</strong> öğrencisinin{" "}
              <strong>
                {aylar[selectedDekont.ay - 1]} {selectedDekont.yil}
              </strong>{" "}
              ayına ait dekontunu reddediyorsunuz.
            </p>
          )}
          <div>
          ...
          <div className="space-y-4">
              ...
              <div className="flex items-start">
                ...
                <div className="ml-3">
                  <h4 className="text-sm font-medium text-red-800 mb-2">
                    ⚠️ Dikkat: Kalıcı Veri Kaybı!
                  </h4>
                  <p className="text-sm text-red-700 mb-2">
                    Bu işlem <strong>geri alınamaz</strong> ve aşağıdaki
                    verilerin <strong>kalıcı olarak kaybına</strong> neden
                    olacaktır:
                  </p>
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              <h5 className="font-medium text-gray-900 mb-2">
                Silinecek Dekont Bilgileri:
              </h5>
              <div className="text-sm text-gray-700 space-y-1">
                <p>
                  <strong>Öğrenci:</strong> {selectedDekont.ogrenci_ad_soyad}
                </p>
                <p>
                  <strong>Dönem:</strong> {aylar[selectedDekont.ay - 1]}{" "}
                  {selectedDekont.yil}
                </p>
                <p>
                  <strong>İşletme:</strong> {selectedDekont.isletme_ad}
                </p>
                {selectedDekont.miktar && (
                  <p>
                    <strong>Tutar:</strong>{" "}
