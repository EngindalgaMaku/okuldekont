...
export default function DekontUpload({
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof DekontFormData, string>>
  >({});
  ...
  // Mobile direct file input change -> auto-submit with session check
  const handleMobileFileChange = async (
    e: React.ChangeEvent<HTMLInputElement>
  ) => {
    const f = e.target.files?.[0] || null;
    if (f) {
      // Mobil için session kontrolü
      try {
        ...
        console.warn("Session check failed on mobile:", error);
        // Continue with file handling despite error
      }
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select
              ...
              value={formData.ay}
              onChange={(e) =>
                setFormData((prev) => ({ ...prev, ay: e.target.value }))
              }
              className="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500 transition-all sm:text-sm"
            >
              {AY_LISTESI.map((ay, index) => {
                ...
                if (startDate) {
                  ...
                  if (
                    selectedYear === stajBaslangicYear &&
                    index + 1 < stajBaslangicMonth
                  ) {
                    return null;
                  }
                }
            ...
            {isMobile ? (
              <div className="mb-2">
                <label
                  className={`w-full flex items-center justify-center gap-2 ${
                    compact ? "p-4" : "p-6"
                  } rounded-lg bg-blue-600 text-white font-medium cursor-pointer active:bg-blue-700`}
                >
              ...
              <div
                ...
                className={`cursor-pointer border-2 border-dashed rounded-lg ${
                  compact ? "p-4" : "p-6"
                } text-center transition-colors mb-2 ${
                  isDragReject
