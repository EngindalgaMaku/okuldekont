...
export default function TeacherHistoryPage() {
  ...
  const [historyData, setHistoryData] = useState<TeacherHistoryItem[]>([])
  const [filteredData, setFilteredData] = useState<TeacherHistoryItem[]>([])
  ...
  const changeTypes = [
    ...
    { value: 'PERSONAL_INFO_UPDATE', label: 'Kişisel Bilgi Güncelleme' },
    { value: 'CONTACT_INFO_UPDATE', label: 'İletişim Güncelleme' },
    { value: 'FIELD_ASSIGNMENT_UPDATE', label: 'Alan Değişikliği' },
    { value: 'STATUS_UPDATE', label: 'Durum Değişikliği' },
    { value: 'POSITION_UPDATE', label: 'Pozisyon Güncelleme' },
    { value: 'OTHER_UPDATE', label: 'Diğer Değişiklikler' }
  ]
  ...
  const fetchTeacherHistory = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Öğretmen geçmişi getirilemedi')
      
      const data = await response.json()
      setHistoryData(data.history || [])
      setFilteredData(data.history || [])
    } catch (error) {
      console.error('Öğretmen geçmişi çekilirken hata:', error)
      alert('Öğretmen geçmişi yüklenirken bir hata oluştu')
    }
    setLoading(false)
  }
  ...
  const exportData = () => {
    ...
    const csvContent = filteredData.map(item => 
      `"${item.teacher_name}","${item.change_type}","${item.changed_field}","${item.old_value || ''}","${item.new_value || ''}","${new Date(item.valid_from).toLocaleDateString('tr-TR')}","${item.description || ''}"`
    ).join('\n')

    const header = 'Öğretmen Adı,Değişiklik Tipi,Değişen Alan,Eski Değer,Yeni Değer,Tarih,Açıklama\n'
    const blob = new Blob([header + csvContent], { type: 'text/csv;charset=utf-8;' })
    const link = document.createElement('a')
    link.href = URL.createObjectURL(blob)
    link.download = `ogretmen-gecmisi-${new Date().toISOString().split('T')[0]}.csv`
    link.click()
  }

  const getChangeTypeColor = (type: string) => {
    switch (type) {
      case 'PERSONAL_INFO_UPDATE': return 'blue'
      case 'CONTACT_INFO_UPDATE': return 'green'
      case 'FIELD_ASSIGNMENT_UPDATE': return 'purple'
      case 'STATUS_UPDATE': return 'orange'
      case 'POSITION_UPDATE': return 'indigo'
      case 'OTHER_UPDATE': return 'yellow'
      default: return 'gray'
    }
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-3xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">
                Öğretmen Geçmiş Takibi
              </h1>
              <p className="text-gray-600 mt-2">Öğretmenlerin bilgi değişikliklerini izleyin</p>
            </div>
          </div>
          <button
            onClick={fetchTeacherHistory}
            disabled={loading}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
            <div>
              ...
              <input
                ...
                placeholder="Öğretmen adı, açıklama..."
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-4">
                        <div className={`p-2 rounded-lg bg-${getChangeTypeColor(item.change_type)}-100`}>
                          <Users className={`h-5 w-5 text-${getChangeTypeColor(item.change_type)}-600`} />
                        </div>
                        <div>
                          <h4 className="font-medium text-gray-900">{item.teacher_name}</h4>
        ...
        <div className="mt-8 bg-gradient-to-r from-purple-50 to-blue-50 rounded-2xl p-6 border border-purple-200">
          <div className="flex items-start">
            <GraduationCap className="h-6 w-6 text-purple-600 mr-3 flex-shrink-0 mt-0.5" />
            <div>
              <h4 className="text-base font-semibold text-purple-900 mb-2">Öğretmen Geçmiş Takibi</h4>
              <div className="text-sm text-purple-800 space-y-1">
                <p>• <strong>Bilgi Güncelleme:</strong> Öğretmen adı, telefon, email değişiklikleri</p>
                <p>• <strong>Alan Değişikliği:</strong> Meslek alanı ve branş değişiklikleri</p>
                <p>• <strong>Durum Değişikliği:</strong> Aktif/pasif durum güncellemeleri</p>
                <p>• <strong>Yeterlilik Güncelleme:</strong> Mesleki yeterlilik belgesi değişiklikleri</p>
                <p>• <strong>Görevlendirme:</strong> İşletme ataması ve görev değişiklikleri</p>
              </div>
            </div>
          </div>
        </div>
