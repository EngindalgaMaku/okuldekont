...
import { Settings, Database, Users, Mail, Shield, Save, RefreshCw, HardDrive, Download, Trash2, Key, RotateCcw, DollarSign, Edit, X, Eraser, AlertTriangle, FileX, BookOpen, Calendar } from 'lucide-react'
            ...
            <div className="lg:col-span-2">
              ...
              <div className="bg-white/80 backdrop-blur-lg shadow-xl rounded-2xl border border-indigo-100 p-4 sm:p-6">
                ...
                <div className="space-y-4 sm:space-y-6">
                  ...
                  <div>
                    <h3 className="text-sm font-medium text-gray-900 mb-3">Eğitim Yılı Yönetimi</h3>
                    <div className="space-y-4">
                      ...
                      {activeEducationYear && (
                        <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-4 border border-green-200">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            <div>
                              <div className="text-lg font-bold text-green-700">{activeEducationYear.year}</div>
                              <div className="text-xs text-green-600">Aktif Eğitim Yılı</div>
                            </div>
                            <div>
                        ...
                        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center">
                              ...
                              <div>
                                <p className="text-sm font-medium text-yellow-800">Aktif dönem bulunamadı</p>
                                <p className="text-xs text-yellow-700">Yeni eğitim yılı oluşturun</p>
                              </div>
                            </div>
                            <button
                              onClick={() => setShowEducationYearModal(true)}
                      ...
                      {educationYears.length > 0 && (
                        <div>
                          <h4 className="text-xs font-medium text-gray-700 mb-2">Tüm Dönemler ({educationYears.length})</h4>
                          <div className="space-y-2">
                            {educationYears.slice(0, 3).map((year) => (
                              <div key={year.id} className={`flex items-center justify-between p-2 rounded-lg text-xs ${
                                year.active
                                ...
                                <div className="flex items-center space-x-1">
                                  {year.active && <span className="px-1 bg-green-200 text-green-800 rounded text-xs">Aktif</span>}
                                  {!year.active && (
                                    <button
                  ...
                  <div>
                    <h3 className="text-sm font-medium text-gray-900 mb-3">Dosya ve Tarih Ayarları</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">Dekont Son Teslim Günü</label>
                        <input type="number" min="1" max="28" value={settings.dekont_son_gun} onChange={(e) => setSettings({ ...settings, dekont_son_gun: parseInt(e.target.value) })} className="block w-full px-3 py-2 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm sm:text-base" />
                        <p className="text-xs text-gray-500 mt-1">Her ayın kaçına kadar dekont yüklenebilir.</p>
                      </div>
                    </div>
                  </div>
