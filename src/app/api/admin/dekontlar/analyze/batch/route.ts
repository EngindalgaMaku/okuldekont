...
export async function POST(request: NextRequest) {
  ...
  if (!authResult.success) {
    await logSecurityEvent(
      "",
      "UNAUTHORIZED_BATCH_ANALYSIS_ATTEMPT",
      {
        ip: clientIp,
        userAgent,
  ...
  try {
    ...
    if (!requestValidation.isValid) {
      await logSecurityEvent(
        userId,
        "INVALID_BATCH_ANALYSIS_REQUEST",
        {
    ...
    if (!rateLimitCheck.isValid) {
      await logSecurityEvent(
        userId,
        "BATCH_ANALYSIS_RATE_LIMIT_EXCEEDED",
        {
          ip: clientIp,
          userAgent,
        },
    ...
    if (!batchValidation.isValid) {
      await logSecurityEvent(
        ...
        "INVALID_BATCH_REQUEST",
        {
          ...
          requestedCount: Array.isArray(dekontIds)
            ? dekontIds.length
            : "invalid",
          ip: clientIp,
        },
    ...
    console.log(
      `🔄 BATCH ANALYSIS: Starting batch analysis for ${dekontIds.length} dekontlar`
    );
    ...
    console.log(
      `📋 BATCH ANALYSIS: Found ${dekontlar.length} dekontlar with files`
    );
    ...
    for (const dekont of dekontlar) {
      try {
        console.log(
          `🔍 BATCH ANALYSIS: Processing dekont ${dekont.id} (${
            processed + 1
          }/${dekontlar.length})`
        );
        ...
        console.error(
          `❌ BATCH ANALYSIS: Failed for dekont ${dekont.id}:`,
          error
        );
        errors.push({
          dekontId: dekont.id,
    ...
    console.log(
      `🤖 BATCH ANALYSIS: Starting AI analysis for ${results.length} processed documents`
    );
    ...
    for (const result of results) {
      const aiAnalysis = aiAnalysisResults.find(
        (ai) => ai.id === result.id
      )?.analysis;
    ...
    if (updatePromises.length > 0) {
      console.log(
        `💾 BATCH ANALYSIS: Updating ${updatePromises.length} dekontlar in database`
      );
    ...
    const summary = {
      ...
      averageReliability:
        finalResults.length > 0
          ? finalResults.reduce((sum, r) => sum + r.reliability, 0) /
            finalResults.length
          : 0,
      recommendations: {
        approve: finalResults.filter((r) => r.recommendation === "APPROVE")
          .length,
        reject: finalResults.filter((r) => r.recommendation === "REJECT")
          .length,
        manualReview: finalResults.filter(
          (r) => r.recommendation === "MANUAL_REVIEW"
        ).length,
      },
    };
