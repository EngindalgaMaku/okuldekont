...
export async function POST(
  ...
  try {
    ...
    const dekont = await prisma.dekont
      .findUnique({
        where: { id },
        include: {
          staj: {
      ...
      .catch((error) => {
        logger.database("dekont_fetch_failed", {
          dekontId: id,
          error: error.message,
          correlationId,
        });
    ...
    logger.info(
      `🔒 FINANCIAL: Dekont ${decision.toLowerCase()} operation initiated`,
      {
        dekontId: id,
        decision,
    ...
    const updatedDekont = await prisma.dekont
      .update({
        where: { id },
        data: {
          status: decision,
    ...
    logger.info(
      `✅ FINANCIAL: Dekont ${decision.toLowerCase()} completed successfully`,
      {
        dekontId: id,
        decision,
        adminId: authResult.user?.id,
        adminEmail: authResult.user?.email,
        studentName: updatedDekont.staj?.student
          ? `${updatedDekont.staj.student.name} ${updatedDekont.staj.student.surname}`
          : "Unknown",
    ...
    const responseData = {
      ...
      amount: updatedDekont.amount ? Number(updatedDekont.amount) : null,
