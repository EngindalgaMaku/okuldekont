...
export async function GET(
  ...
  try {
    ...
    const transformedIsletmeler = isletmelerData.map((isletme: any) => {
      ...
      const stajOgrencileri = isletme.stajlar.map((staj: any) => ({
        id: `staj-${staj.id}`,
      ...
      const atanmisOgrenciler = isletme.students
        .filter(
          (student: any) =>
            !isletme.stajlar.some((staj: any) => staj.student.id === student.id)
        )
        .map((student: any) => ({
      ...
      if (isletme.stajlar.length > 0) {
        ...
        const firstInternshipTeacher = isletme.stajlar.find(
          (staj: any) => staj.teacher
        )?.teacher;
