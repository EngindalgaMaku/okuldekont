...
export async function GET(
  ...
  try {
    ...
    const educationYearId =
      qEducationYearId || (await getActiveEducationYearId());

    console.log(
      "🛡️ ADMIN SECURITY: Authorized admin accessing company dekontlar:",
      {
...
export async function POST(
  ...
  try {
    ...
    console.log(
      "🛡️ FILE SECURITY: Starting secure admin company dekont upload:",
      {
        adminId: authResult.user?.id,
        adminEmail: authResult.user?.email,
        companyId,
    ...
    if (dosya && dosya.size > 0) {
      ...
      if (!securityResult.safe) {
        ...
        console.error(
          "🚨 FILE SECURITY: Malicious admin company dekont blocked:",
          {
            fileName: dosya.name,
            adminId: authResult.user?.id,
            companyId,
      ...
      console.log(
        "✅ FILE SECURITY: Admin company dekont passed security scan"
      );
      ...
      try {
        ...
        console.log(
          "✅ FILE SECURITY: Secure admin company dekont upload completed:",
          {
            originalName: dosya.name,
    ...
    const formattedDekont = {
      ...
      miktar: newDekont.amount
        ? Number(decryptFinancialData(newDekont.amount.toString()))
        : null,
      ...
      stajlar: {
        ogrenciler: {
          ...
          alanlar: student?.alan
            ? { ad: student.alan.name }
            : { ad: "Bilinmiyor" },
        },
      },
    };
