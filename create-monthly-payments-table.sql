-- Monthly Payments tablosu oluşturmak için SQL
-- Bu dosyayı veritabanında manuel çalıştırın
...
ALTER TABLE `monthly_payments` ADD CONSTRAINT `monthly_payments_companyId_fkey` FOREIGN KEY (`companyId`) REFERENCES `companies`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

ALTER TABLE `monthly_payments` ADD CONSTRAINT `monthly_payments_teacherId_fkey` FOREIGN KEY (`teacherId`) REFERENCES `teachers`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;

ALTER TABLE `monthly_payments` ADD CONSTRAINT `monthly_payments_stajId_fkey` FOREIGN KEY (`stajId`) REFERENCES `internships`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;

ALTER TABLE `monthly_payments` ADD CONSTRAINT `monthly_payments_educationYearId_fkey` FOREIGN KEY (`educationYearId`) REFERENCES `education_years`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;